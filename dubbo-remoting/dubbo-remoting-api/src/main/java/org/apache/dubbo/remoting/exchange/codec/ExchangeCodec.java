...
//    请求对象的编码过程，该过程首先会通过位运算将消息头写入到 header 数组中。
//    然后对 Request 对象的 data 字段执行序列化操作，序列化后的数据最终会存储到
//    ChannelBuffer 中。序列化操作执行完后，可得到数据序列化后的长度 len，紧接着
//    将 len 写入到 header 指定位置处。最后再将消息头字节数组 header 写入到
//    ChannelBuffer 中，整个编码过程就结束了
